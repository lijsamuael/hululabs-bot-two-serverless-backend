...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  try {
    ...
    if (!type || !data) {
      return res
        .status(400)
        .json({ error: "Missing required fields: type and data" });
    }
    ...
    if (type === "order" && !("order_number" in data)) {
      return res
        .status(400)
        .json({ error: "Order statistics must include order_number" });
    }
    ...
    //   throw new Error(
    //     `API returned status ${response.status}: ${response.data}`
    //   );
    // }
